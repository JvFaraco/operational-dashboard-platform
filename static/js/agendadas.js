...
.then(data => {
    ...
    fetch('/api/meta/agendadas').then(r=>r.json()).then(meta=>{
      const m = document.getElementById('metaInfo');
...
const dias = parseInt(
  ...
  // Converter data Agenda para string YYYY-MM-DD
let agendamento = String(
  r['Agenda']
  ?? r['Agendamento']
  ?? r['Data Agendamento']
  ?? ''
  ...
  const defeito=String(r['Defeito']??r['Motivo']??'').trim();
  ...
  // Extrair nome limpo do técnico (formato: GNA - A365 - EPV - Nome)
  const parts=tec.split('-').map(s=>s.trim());
  const nomeTec=parts.length>=4?parts.slice(3).join(' ').trim():tec;
  const tipoOsMap={'Nova instalação':'Instalação','A365 Reativação Contratual':'Instalação','A365 Repareamento':'Instalação','A365 Troca de Tecnologia':'Instalação','A365 Manutenção Técnica':'Manutenção','A365 Intrusão':'Manutenção','A365 Reentrega':'Manutenção','A365 Remanejar Equipamento':'Manutenção','Falha de comunicação de dispositivo':'Manutenção','Falha de comunicação de central':'Manutenção','A365 - Cancelamento por Inadimplência':'Desinstalação','A365 Administrativa de Apoio':'Administrativa','A365 Colagem de Placa':'Administrativa','A365 Mudança de Endereço':'Manutenção','A365 Retirada de Equipamentos':'Desinstalação','A365 Troca de Titularidade':'Desinstalação','Cancelamento de contrato':'Desinstalação'};
  const tipoOS=tipoOsMap[defeito]??'Manutenção';
  return{
    'Nº OS':String(r['Nº OS']??'').trim(),
...
function renderTabela(){
  ...
  document.getElementById('detBody').innerHTML=rows.map(r=>`<tr>
    ...
    <td style="color:var(--muted);white-space:nowrap;">${esc(r.Agendamento)}</td>
    <td style="max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="${esc(r.Defeito)}">${esc(r.Defeito)}</td>
    <td style="max-width:260px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="${esc(r.Técnico)}">${esc(r.Técnico)}</td>
    <td>${tag(r.Tipo)}</td>
...
document.getElementById('exportBtn').addEventListener('click',()=>{
  ...
  out.push([q('Nº OS'), q('Agendamento'), q('Tipo de O.S'), q('Motivo/Defeito'), q('Técnico'), q('Tipo Técnico'), q('Dias'), q('Faixa'), q('Cliente'), q('Conta')].join(sep));
  filteredData.forEach(r=>{
    out.push([q(r['Nº OS']), q(r.Agendamento), q(r.TipoOS), q(r.Defeito), q(r.Técnico), q(r.Tipo), q(r.Dias), q(r.Bucket), q(r.Fantasia), q(r.Conta)].join(sep));
  });

  const blob = new Blob(['\uFEFF' + out.join('\n')], {type:'text/csv;charset=utf-8;'});
  const a = document.createElement('a');
  a.href = URL.createObjectURL(blob);
  a.download = 'OS-Agendadas-' + new Date().toISOString().slice(0,10) + '.csv';
  a.click();
