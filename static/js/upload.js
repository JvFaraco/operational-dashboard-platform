const LABELS = {
  ...
  base:          'Base Compartilhada (Contas x EPV)'
};

// Carrega meta das 3 bases de dashboard + base compartilhada
function renderMeta(){
  fetch('/api/meta').then(r => r.json()).then(metas => {
    ...
    // Base compartilhada
    const baseInfo = document.getElementById('metaBaseInfo');
    if(baseInfo){
      const m = metas['base'];
      if(m && m.registros){
        baseInfo.textContent =
          m.arquivo + ' · ' +
          m.registros.toLocaleString('pt-BR') + ' registros · ' +
          m.atualizado_em +
          (m.usuario ? ' · ' + m.usuario : '');
        baseInfo.className = '';
      } else {
        baseInfo.textContent = 'Sem dados carregados ainda.';
        baseInfo.className = 'meta-none';
      }
    }
  }).catch(() => {});
}
...
btnUp.addEventListener('click', async () => {
  ...
  try {
    ...
    if(data.ok){
      const unidade = tipo === 'base' ? 'registros' : 'O.S';
      progLabel.textContent = data.registros.toLocaleString('pt-BR') + ' ' + unidade + ' carregados com sucesso!';
      toast(data.registros.toLocaleString('pt-BR') + ' ' + unidade + ' carregados em ' + LABELS[tipo] + '!', 'ok');
      renderMeta();
