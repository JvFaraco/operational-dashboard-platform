...
let activeBucket='', activePausada='', activeEspec='', activeSupervisor='';
const sel={regiao:new Set()};
let bucketChart=null, regionalChart=null;

const BKEYS=['0-2','3-14','14+'];
const BLABELS=['0-2 dias','3-14 dias','+14 dias'];
const BCOLORS={'0-2':'#2db86a','3-14':'#4d9fdd','14+':'#e05050'};
const LS_KEY='bi_recred_state';

const SUPERVISORES = {
  'Átilas Souza':   ['ITP','SOO','PAE','CCO','LGS'],
  'Alexandre Reis': ['CTA','JLE','IAI','BNU','BQE','JGS'],
  'Rodrigo Lodo':   ['CAS','SRR','RBP','PMJ','GNA','BSB','SPO','BHE']
};
function siglaSupervisor(sig){ for(const sup in SUPERVISORES){ if(SUPERVISORES[sup].includes(sig)) return sup; } return ''; }

let EMBED_CONTAS=[], CONTAS_POR_SIGLA={}, CONTAS_TOTAL=0;
function recomputeContas(){
  CONTAS_POR_SIGLA={}; CONTAS_TOTAL=0;
  EMBED_CONTAS.forEach(c=>{
    if(!CONTAS_POR_SIGLA[c.sigla]) CONTAS_POR_SIGLA[c.sigla]=0;
    CONTAS_POR_SIGLA[c.sigla]+=c.contas; CONTAS_TOTAL+=c.contas;
  });
}

// ── LOAD ──────────────────────────────────────────────────────────────────────
Promise.all([
  fetch('/api/recred').then(r=>r.json()),
  fetch('/api/base').then(r=>r.json()).catch(()=>[])
])
.then(([data, baseData])=>{
  if(baseData && baseData.length){
    EMBED_CONTAS = baseData.map(row=>({
      sigla: (String(row['TecnicoResponsavel']||'')).split(' - ')[1]?.trim()||'',
      tecnico: String(row['TecnicoResponsavel']||''),
...
function norm(r){
  ...
  const regional=extrairRegional(tec);
  return{
    'Nº OS':os,
    'Téc. Executor':tec,
    'Regional':regional,
    'Supervisor':siglaSupervisor(regional),
...
// ── SUPERVISOR ────────────────────────────────────────────────────────────────
function regionaisPermitidas(){
  if(activeSupervisor) return new Set(SUPERVISORES[activeSupervisor]);
  return null;
}

function buildSupervisorCards(){
  const host=document.getElementById('supCards');
  if(!host) return;
  const ordem=['Alexandre Reis','Rodrigo Lodo','Átilas Souza'];
  host.innerHTML = ordem.map(sup=>{
    const regs=SUPERVISORES[sup];
    const osCnt=rawData.filter(r=>r['Supervisor']===sup).length;
    const contas=regs.reduce((a,s)=>a+(CONTAS_POR_SIGLA[s]||0),0);
    const pct=contas>0?((osCnt/contas)*100).toFixed(1).replace('.',',')+'%':'—';
    const multa=rawData.filter(r=>r['Supervisor']===sup).reduce((s,r)=>s+(r['Multa']||0),0);
    const pausadas=rawData.filter(r=>r['Supervisor']===sup&&r['Pausada?']==='SIM').length;
    return `<div class="supcard" data-sup="${esc(sup)}">
      <div class="supname">${esc(sup)}</div>
      <div class="supregs">${regs.join(' · ')}</div>
      <div class="supstats">
        <span><b>${osCnt.toLocaleString('pt-BR')}</b> OS</span>
        <span><b>${contas.toLocaleString('pt-BR')}</b> contas</span>
        <span><b>${pct}</b> OS/conta</span>
        ${pausadas>0?`<span><b>${pausadas}</b> pausadas</span>`:''}
        ${multa>0?`<span><b>R$ ${multa.toLocaleString('pt-BR',{maximumFractionDigits:0})}</b> exposto</span>`:''}
      </div>
    </div>`;
  }).join('');
  host.querySelectorAll('.supcard').forEach(c=>c.addEventListener('click',()=>{
    activeSupervisor=activeSupervisor===c.dataset.sup?'':c.dataset.sup;
    sel.regiao.clear();
    setupFilters();
    applyFilters();
  }));
...
function updateSupCards(){
  document.querySelectorAll('.supcard').forEach(c=>c.classList.toggle('active',c.dataset.sup===activeSupervisor));
  const back=document.getElementById('backOverview');
  const ttl=document.getElementById('viewTitle');
  if(back) back.style.display=activeSupervisor?'inline-flex':'none';
  if(ttl) ttl.textContent=activeSupervisor?('Supervisor: '+activeSupervisor):'Visão Geral';
}

// ── FILTERS ───────────────────────────────────────────────────────────────────
function applyFilters(){
  const perm=regionaisPermitidas();
  filteredData=rawData.filter(r=>{
    const supOk=!perm||perm.has(r['Regional']);
...
function updateFilterUI(){
  ...
  const parts=[];
  if(activeSupervisor){parts.push('Sup: '+activeSupervisor);dot.style.background='var(--accent)';}
...
function renderKPIs(){
  ...
  const mais15=filteredData.filter(r=>r['Bucket']==='14+').length;
  ...
  // Contas ativas e % OS/Conta (base compartilhada)
  const perm=regionaisPermitidas();
  let contasSel=perm?[...perm].reduce((a,s)=>a+(CONTAS_POR_SIGLA[s]||0),0):CONTAS_TOTAL;
  if(sel.regiao.size>0) contasSel=[...sel.regiao].reduce((a,s)=>a+(CONTAS_POR_SIGLA[s]||0),0);
  animN(document.getElementById('kpiContas'),contasSel);
  const pctEl=document.getElementById('kpiPct');
  if(pctEl) pctEl.textContent=contasSel>0?((total/contasSel)*100).toFixed(1).replace('.',',')+'%':'—';
  const pctSub=document.getElementById('kpiPctSub');
  if(pctSub) pctSub.textContent=total.toLocaleString('pt-BR')+' OS ÷ '+contasSel.toLocaleString('pt-BR');
}

// ── RENDER BUCKET CHART ───────────────────────────────────────────────────────
function renderBucket(){
  const cnt={'0-2':0,'3-14':0,'14+':0};
...
function renderRegional(){
  ...
  regionalChart=new Chart(canvasEl,{
    ...
    data:{
      ...
      datasets:[{
        data:vals,
        backgroundColor:selKeys.map(k=>sel.regiao.has(k)?'rgba(224,80,80,1)':'rgba(224,80,80,.75)'),
        borderColor:selKeys.map(k=>sel.regiao.has(k)?'#e05050':'rgba(224,80,80,.4)'),
        borderWidth:selKeys.map(k=>sel.regiao.has(k)?2:1),
        borderRadius:5,
        borderSkipped:false
      }]
    },
    options:{
      ...
      onClick:(e,els)=>{
        ...
        const key=selKeys[els[0].index];
        if(sel.regiao.has(key)&&sel.regiao.size===1){sel.regiao.clear();}
        else{sel.regiao.clear();sel.regiao.add(key);}
...
document.getElementById('backOverview').addEventListener('click',()=>{
  activeSupervisor='';sel.regiao.clear();setupFilters();applyFilters();
});

document.getElementById('clearFiltersBtn').addEventListener('click',()=>{
  activeBucket='';activePausada='';activeEspec='';activeSupervisor='';
  sel.regiao.clear();
